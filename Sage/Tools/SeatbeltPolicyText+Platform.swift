//
//  SeatbeltPolicyText+Platform.swift
//  Sage
//
...
extension SeatbeltPolicyText {
    ...
    ; ------------------------------------------------------------------
    ; Preferences IPC (codex seatbelt_preferences_policy.sbpl)
    ; ------------------------------------------------------------------
    ...
    ; ------------------------------------------------------------------
    ; Sage: unrestricted reads, matching the pre-sandbox shell. Writes
    ; stay deny-default; writable roots are appended per invocation.
    ; ------------------------------------------------------------------
    (allow file-read* file-test-existence)
    (allow file-map-executable)

    ; Per-user temp domain ($TMPDIR lives here).
    """
}
